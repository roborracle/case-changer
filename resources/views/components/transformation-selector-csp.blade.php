...
<div x-data="transformationSelector" 
     x-init="initWithData(@js($allTools), @js($currentTool))"
    ...
    <div class="mb-4">
        ...
        <div class="flex flex-wrap gap-2">
            @foreach($popularTools as $index => $tool)
                <button
                    @click="selectTool('{{ $tool['id'] }}', '{{ $tool['name'] }}')"
                    :class="getQuickToolClasses('{{ $tool['id'] }}')"
            ...
            <div class="relative" @keydown.escape="closeDropdown">
                <button
                    ...
                    <span class="text-xs bg-white/20 dark:bg-white/10 px-2 py-0.5 rounded-full">
                        <span x-text="toolsCount"></span>
                    </span>
                    <svg class="w-4 h-4 transition-transform duration-200" 
                         :class="dropdownRotateClass"
                        ...
                        <div class="sticky top-0 p-3 border-b border-gray-200 dark:border-gray-700 bg-white/90 dark:bg-gray-900/90 backdrop-blur">
                            <div class="relative">
                                ...
                                <input type="text"
                                       ...
                                       @input="searchQuery = $event.target.value; filterTools()"
                                       x-ref="searchInput"
                                    ...
                                    <div class="space-y-1">
                                        <template x-for="tool in category" :key="tool.id">
                                            <button
                                                ...
                                                :class="isToolSelected(tool.id) ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400' : 'hover:bg-gray-100 dark:hover:bg-gray-800'"
                                                ...
                                                <template x-if="isToolSelected(tool.id)">
    ...
    <div class="sr-only" aria-live="polite" aria-atomic="true">
        <span x-text="selectedToolDisplay"></span>
    </div>
</div>