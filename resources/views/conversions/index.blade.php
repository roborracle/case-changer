...
@section('description', 'The ultimate text transformation tool. Convert text to any case format instantly - uppercase, lowercase, camelCase, snake_case, title case, and 170+ more formats. Free, fast, and professional.')
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <div x-data="universalConverter()" class="rounded-lg p-6 bg-primary border" >
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        @input="transform"
                        rows="10"
                        class="w-full p-4 rounded-lg border focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-secondary text-primary"
                ...
                <div x-show="isLoading" class="mt-4 text-center text-secondary" >Converting...</div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="flex items-start justify-between mb-4">
                ...
                <span class="text-xs px-2 py-1 rounded text-tertiary bg-secondary" >
                {{ count($category['tools']) }} tools
                </span>
            </div>

            <h2 class="text-xl font-semibold mb-2 transition-colors text-primary">
            {{ $category['title'] }}
            </h2>
            ...
            <div class="flex flex-wrap gap-2">
                ...
                <span class="text-xs px-2 py-1 rounded bg-secondary text-secondary" >
                {{ $tool['name'] }}
                </span>
                @endforeach
        ...
        <div class="mt-16 rounded-xl p-8 bg-secondary" >
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
                ...
                class="px-4 py-2 rounded-lg text-center hover:shadow transition-all bg-primary border hover-border-accent">
                <span class="text-sm font-medium text-primary" >UPPERCASE</span>
                </a>
                <a href="{{ route('conversions.tool', ['case-conversions', 'lowercase']) }}"
                class="px-4 py-2 rounded-lg text-center hover:shadow transition-all bg-primary border hover-border-accent">
                <span class="text-sm font-medium text-primary" >lowercase</span>
                </a>
                <a href="{{ route('conversions.tool', ['developer-formats', 'camel-case']) }}"
                class="px-4 py-2 rounded-lg text-center hover:shadow transition-all bg-primary border hover-border-accent">
                <span class="text-sm font-medium font-mono text-primary" >camelCase</span>
                </a>
                <a href="{{ route('conversions.tool', ['developer-formats', 'snake-case']) }}"
                class="px-4 py-2 rounded-lg text-center hover:shadow transition-all bg-primary border hover-border-accent">
                <span class="text-sm font-medium font-mono text-primary" >snake_case</span>
                </a>
                <a href="{{ route('conversions.tool', ['journalistic-styles', 'ap-style']) }}"
                class="px-4 py-2 rounded-lg text-center hover:shadow transition-all bg-primary border hover-border-accent">
                <span class="text-sm font-medium text-primary" >AP Style</span>
                </a>
                <a href="{{ route('conversions.tool', ['academic-styles', 'apa-style']) }}"
                class="px-4 py-2 rounded-lg text-center hover:shadow transition-all bg-primary border hover-border-accent">
                <span class="text-sm font-medium text-primary" >APA Style</span>
                </a>
            </div>
        </div>
    </div>
